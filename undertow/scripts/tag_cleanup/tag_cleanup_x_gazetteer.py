"""
Performer-name gazetteer builder: fetches performer names + known aliases from ThePornDB
and/or StashDB, normalizes each one the same way tag_cleanup_x.py tokenizes tags (so
gazetteer phrases match tag tokens exactly), and writes the merged result to
performer-gazetteer.json (tag_cleanup_x_engine.py's PERFORMER_GAZETTEER_CACHE_FILE).

tag_cleanup_x.py's optional name-detection pass only ever reads that cache file - it never
calls these APIs itself, so name detection stays off until this has been run at least once.

Hard dependency: requests. Launched via the standalone scripts/performer_gazetteer.py entry
point, which just adds this folder to sys.path and calls main() here.
"""
