...
Shared with tag_cleanup.py: the editable word lists (`tag_cleanup_lists.py`, edited via the
...
run. HTML reports and the preview log are likewise written under their own `-x-` names.
...
def write_html_report(previews: List[FilePreview], title: str, summary_note: str) -> Path:
    ...
    out_path = HTML_OUTPUT_DIR / f"tag-cleanup-x-{stamp}.html"
...
<div class="page">
  <header>
    <p class="eyebrow">tag_cleanup_x.py</p>
  ...
  <footer class="note">Generated {datetime.datetime.now().isoformat(timespec="seconds")} by tag_cleanup_x.py. Local file only - nothing here is uploaded anywhere.</footer>
</div>
</body>
</html>
"""
...
def run_dry_run_then_apply(client: HydrusClient, cfg: Config, services: ServiceSelection) -> int:
    ...
    log_path = LOCAL_CONFIG_FILE.parent / "tag-cleanup-x-last-preview.txt"
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    print("=== Hydrus filename-tag cleanup X ===")
