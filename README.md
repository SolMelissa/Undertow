...
**First time on this machine?** Run `scripts/Setup-HydrusPipeline.ps1` from a *non-admin* PowerShell
...
per-site parallel, etc.) is in [`docs/PYTHON_PORT_SETUP.md`](docs/PYTHON_PORT_SETUP.md) — note that doc
...
Everything above is the active codebase. `legacy/Launch-HydrusPipeline.ps1`,
`legacy/Configure-ApiKeys.ps1`, `legacy/Stop-HydrusPipelineServices.ps1`, and
`legacy/Create-DesktopShortcut.ps1` are the **legacy PowerShell originals** this package
replaced — left in place only as a fallback, not where feature work happens.
`scripts/Setup-HydrusPipeline.ps1` is the one PowerShell script still in active use (see
Quick start above).
...
- [`docs/PYTHON_PORT_SETUP.md`](docs/PYTHON_PORT_SETUP.md) — full daily-use walkthrough,
...
- [`CLAUDE.md`](CLAUDE.md) — contributor-facing guide to what to edit for what kind of change,
  ...
  workers).
- [`docs/Hydrus_Pipeline_Guide.docx`](docs/Hydrus_Pipeline_Guide.docx) / `.html` — end-user guide
  to using *Hydrus itself* (searching, tagging, folder-to-tag workflows), generated by
  `scripts/build_guide.js`. Unrelated to this app's own code — it documents the media manager
  you're driving.
