...
Everything above is the active codebase. `Launch-HydrusPipeline.ps1`, `Configure-ApiKeys.ps1`,
`Stop-HydrusPipelineServices.ps1`, and `Create-DesktopShortcut.ps1` are the **legacy PowerShell
originals** this package replaced — left in place only as a fallback, not where feature work
happens. `Setup-HydrusPipeline.ps1` is the one PowerShell script still in active use (see
Quick start above).
...
- [`PYTHON_PORT_SETUP.md`](PYTHON_PORT_SETUP.md) — full daily-use walkthrough, keybinding
  rationale, throttling/parallel-download details, and the history of why this was ported from
  PowerShell.
- [`CLAUDE.md`](CLAUDE.md) — contributor-facing guide to what to edit for what kind of change,
  ...
  workers).
- [`Hydrus_Pipeline_Guide.docx`](Hydrus_Pipeline_Guide.docx) / `.html` — end-user guide to using
  *Hydrus itself* (searching, tagging, folder-to-tag workflows), generated by `build_guide.js`.
  Unrelated to this app's own code — it documents the media manager you're driving.
