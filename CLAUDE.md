...
the `undertow/` Python package (entry point `python -m undertow` via `run.bat`). **The `.ps1`
scripts for daily use are legacy** — untouched, kept only as a fallback. Exception:
`Setup-HydrusPipeline.ps1` is still the active first-time install/provisioning tool and was
intentionally left out of the port — don't route setup questions to the Python package.

The legacy PS1s, the setup script, and the docs all live at the repo root (no
`legacy/`/`scripts/`/`docs/` subfolders).
...
api_client.py, api_keys.py, config.py, tui/), and where the setup script / user guide / dev
